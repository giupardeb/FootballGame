class Portiere extends Giocatore
{
	...
	public Portiere(String cognome,String nazionalita,String squadra,byte velocita,byte resistenza,byte forza,byte morale,byte eta,byte condizione,byte tecnica,byte aggressivita,byte creativita,byte decisione,byte carisma,String posizione,byte giocoDiSquadra,boolean essenziale,byte mediaVoti,byte dotiAeree,byte agilita,byte comunicazione,byte bloccareTiri,byte calciare,byte riflessi,byte rimessa,byte valoreGenerale,double valoreMercato)
	{
	...
	{
		int media=0;
		media=(getVelocita() + getResistenza() + getForza() + getMorale() + getCondizione() + getTecnica() + getAggressivita() + getCreativita() + getDecisione() + getCarisma() + getGiocoDiSquadra() + getDotiAeree() + getAgilita() + getComunicazione() + getBloccareTiri() + getCalciare() + getRiflessi()+ getRimessa() );
		media=(byte)(media/18);
	}
	...
	//	boolean trovato = false;
		...
		for(int i=k+1; i<array.length /*&& trovato !=true*/;i++){
			if(array[i] instanceof Portiere ){
				somma = ((Portiere)array[i]).getValoreGenerale();
